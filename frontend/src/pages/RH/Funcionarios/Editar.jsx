...
export default function FuncionarioEditar() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await api.put(`/rh/funcionarios/${id}`, form)
      navigate(`/rh/funcionarios/${id}`)
    } catch (err) {
      setError(err.message || 'Erro ao actualizar funcionário.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <PageHeader title="Editar Funcionário" subtitle="Actualize os dados do colaborador" />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-outline-variant p-5 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-3">
            {ESTADOS.map(e => (
              <label key={e} className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all ${
                form.estado === e ? 'border-primary bg-primary/5' : 'border-outline-variant hover:border-outline'
              }`}>
                <input type="radio" name="estado" value={e} checked={form.estado === e} onChange={handleChange} className="sr-only" />
                <span className={`w-3 h-3 rounded-full ${e === 'activo' ? 'bg-green-500' : e === 'suspenso' ? 'bg-yellow-500' : 'bg-red-500'}`} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="estado_civil" value={form.estado_civil} onChange={handleChange} className={inputCls}>
                <option value="">Seleccionar</option>
                {ESTADOS_CIVIS.map(e => <option key={e} value={e}>{e.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</option>)}
              </select>
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl border border-outline-variant p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Nº Funcionário</label>
              <input name="numero_funcionario" value={form.numero_funcionario} onChange={handleChange} placeholder="Ex: FUNC-001" className={inputCls} />
            </div>
            <div>
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={() => navigate(`/rh/funcionarios/${id}`)}
